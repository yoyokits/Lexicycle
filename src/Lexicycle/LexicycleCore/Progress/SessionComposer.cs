...
///
/// The point is that consecutive sessions ask different things: words already answered
/// correctly are not served again until the <see cref="ReviewSchedule"/> says they are
/// due, and the rest of each session is filled with words never seen before. Sets that
/// come from outside — the bundled JSON, and later an OCR'd page — bypass this entirely
/// and drill exactly the words they were given.
/// </summary>
public sealed class SessionComposer
{
        ...
        IEnumerable<int> unseenWordIds)
    {
        ...
        var due = seen
            .Where(progress => ReviewSchedule.IsDue(progress, sessionNumber))
